...
const OnboardVendor = () => {
  ...
  const handleFinalSubmit = async () => {
    ...
    try {
      // Ensure the user is logged in. Vendors must be associated
      // with an authenticated account.
      const { data: { user } } = await supabase.auth.getUser();
      if (!user) throw new Error('No authenticated user. Please log in and try again.');

      // Upsert vendor record for this user. If a vendor profile
      // already exists for the user, update it; otherwise insert a new one.
      ...
      const vendorPayload = {
        ...
        niche: vendorData.niche || null,
        pitch: vendorData.description || '',
        status: 'pending'
      };
      ...
      if (!vendorId) throw new Error('Failed to obtain vendor ID.');

      // Create a new product associated with this vendor
      const slug = vendorData.productTitle
        ? vendorData.productTitle.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '')
        : '';

      const productPayload = {
        vendor_id: vendorId,
        name: vendorData.productTitle,
        description: vendorData.productPitch,
        slug: slug,
        status: 'pending'
      };
      ...
      toast({
        ...
        description: "Your vendor application has been submitted for review. We'll contact you soon.",
      });
      // Redirect to vendor dashboard after successful submission
      navigate("/vendor/dashboard");
    } catch (error: any) {
      console.error('Error during vendor submission:', error);
      toast({
        title: 'Error',
