...
const PartnerDashboard = () => {
  ...
  // Get the most recent non‑abandoned application for profile display. The
  // partner_applications table only allows 'in_progress', 'completed'
  // or 'abandoned' statuses. Previously the UI looked for custom
  // statuses like 'submitted', 'approved' or 'profile_completed' which
  // could never match and resulted in no application being shown.
  // We instead pick the most recent application (already sorted in
  // descending order by created_at) that is not abandoned.
  const profileApplication = applications?.find(app => app.status !== 'abandoned');

  // Mock performance data (in real app, this would come from actual sales data)
  const performance = {
    total_sales: 0,
    total_commission: 0,
    // No 'approved' status exists for partner applications; default to
    // counting completed applications as active partnerships.
    active_deals: applications?.filter(app => app.status === 'completed').length || 0,
    conversion_rate: 0
  };
                        ...
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            <h3 className="text-lg font-semibold text-gray-900">
                              {app.product_id === null ? 'General Profile' : 'Product Application'}
                            </h3>
