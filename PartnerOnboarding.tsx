...
export const PartnerOnboarding: React.FC<PartnerOnboardingProps> = ({
  ...
  const loadExistingApplication = async () => {
    try {
      ...
      // The partner_applications table enforces a limited set of
      // statuses (in_progress, completed, abandoned). In the
      // original implementation the status "submitted" was used
      // which does not satisfy the database check constraint and
      // caused inserts/updates to silently fail.  Here we look for
      // the latest completed application instead.
      ...
      if (completedApp && !error) {
        ...
        const newApplicationData = {
          ...
          product_id: productId,
          // Use the allowed status value "completed" when re-submitting
          status: 'completed',
  ...
  const submitApplication = async () => {
    setIsLoading(true);
    try {
      // Update the application status to 'completed'. The partner_applications
      // table defines a CHECK constraint that only allows
      // 'in_progress', 'completed' or 'abandoned'. Previously the
      // code used 'submitted', which violated this constraint and
      // prevented the record from updating. Setting the status to
      // 'completed' ensures the update succeeds and the dashboard
      // can display the finished application.
      const { error } = await supabase
        .from("partner_applications")
